...
<!--
앞 슬라이드가 계층 간 의존성 방향을 설명했다면,
이 슬라이드는 실제 폴더와 파일이 어떻게 배치되는지 보여줍니다.
새 전투 화면이 필요하면 presentation/battle/ 안에만 파일을 추가하면 됩니다.
게임 규칙을 바꾸고 싶으면 domain/battle_engine.dart만 수정하면 되고,
다른 계층은 건드릴 필요가 없습니다.
-->
